...
const GoldenNetworkBackground: React.FC = () => {
  ...
  const { scene, camera, rings, nodes, lines, particles, trails } =
    useMemo(() => {
      ...
      for (let r = 0; r < RING_COUNT; r++) {
        const radius =
          minRadius + (maxRadius - minRadius) * (r / (RING_COUNT - 1));
      ...
      for (let i = 0; i < PARTICLE_COUNT; i++) {
        ...
        const radius =
          minRadius + (maxRadius - minRadius) * (ringIdx / (RING_COUNT - 1));
  ...
  const trailPositions = trails.geometry.attributes.position
    .array as Float32Array;
  ...
  for (let i = 0; i < PARTICLE_COUNT; i++) {
    ...
    positions[i * 3 + 1] =
      baseY + Math.sin(x * 0.7 + t) * Math.cos(z * 0.6 + t * 0.9) * 0.55;
  }
  ...
  for (let i = 0; i < TRAIL_COUNT; i++) {
    ...
    trailPositions[i * 3 + 1] =
      baseY + Math.sin(x * 0.5 + t * 1.3) * Math.cos(z * 0.4 + t * 1.1) * 0.7;
  }
  ...
  return (
    ...
    >
      ...
      <AbsoluteFill
        style={{
          background:
            "radial-gradient(circle at 50% 50%, rgba(212,175,55,0.1) 0%, rgba(5,5,5,0.95) 60%, rgba(5,5,5,1) 100%)",
          pointerEvents: "none",
          mixBlendMode: "screen",
        }}
      />
      <AbsoluteFill
        style={{
          background:
            "radial-gradient(circle at 50% 50%, rgba(0,0,0,0) 35%, rgba(0,0,0,0.85) 100%)",
          pointerEvents: "none",
        }}
      />
    </AbsoluteFill>
